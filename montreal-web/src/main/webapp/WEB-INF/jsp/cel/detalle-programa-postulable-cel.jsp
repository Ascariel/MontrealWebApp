  ...
  <div class="col-md-6 offset-md-3">
    <form method="POST" action="/cel/crear-postulacion-cel-programa?id=${programaDTO.idPrograma }">
    	<c:if test="${postulacionCelPrograma.getIdPostulacion() != null }">
    		<br/>
    		<button type="submit" class="btn btn-md btn-block ${postulacionCelPrograma.textoBtnEstadoPostulacion()} }" disabled> Estado Postulación: ${postulacionCelPrograma.getEstadoPostulacionDTO().getNombreEstado()} </button>
    		<br/><br/>
    	</c:if>
      ...
      <br>
		...
		<c:if test="${postulacionMismoProgramaExiste}">
			<button type="submit" class="btn btn-primary btn-md btn-block" disabled> Postulación Ya Realizada</button>
